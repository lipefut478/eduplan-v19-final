...
import {
  BookOpen, Calendar, BarChart2, ClipboardList, Target, BookMarked,
  FolderOpen, LogOut, Moon, Sun, Menu, X,
} from 'lucide-react';
...
export default function FootballApp() {
  ...
  const [currentModule, setCurrentModule] = useState('micro');
  ...
  const groups = ['Planejamento', 'Ferramentas'];
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: bg, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            ...
            <div style={{ padding: '20px 20px 16px', borderBottom: `1px solid ${sidebarBorder}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: textMuted }}>Planejador de Treinos</div>
                </div>
              </div>
            </div>
      ...
      <div style={{ flex: 1, marginLeft: sidebarOpen ? 240 : 0, transition: 'margin-left 0.2s', minHeight: '100vh' }}>
        ...
        <main style={{ minHeight: 'calc(100vh - 56px)' }}>
          ...
          {currentModule === 'micro' && (
            <MicroPlano session={session} isDark={isDark} metodologia={metodologiaData} />
          )}
          {currentModule === 'lousa' && (
